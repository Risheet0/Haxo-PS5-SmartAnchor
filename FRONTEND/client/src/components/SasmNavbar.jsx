        ...
        <div className="flex items-center gap-6 lg:gap-8">
          ...
          <div className="relative">
            <button
              ...
              className="flex items-center gap-1.5 px-3 py-1.5 rounded-full bg-slate-100 hover:bg-slate-200 text-slate-800 text-xs font-mono font-bold border border-slate-200 transition"
              title="Select priority discovery city"
            >
            ...
            {/* City Dropdown Menu */}
            {isCityDropdownOpen && (
              <div className="absolute left-0 mt-2 w-48 rounded-2xl bg-white border border-slate-200 shadow-xl py-2 z-50 font-mono text-xs space-y-0.5">
                ...
                {SUPPORTED_CITIES.map((city) => (
                  ...
                  >
                    <span>{city.name}</span>
                    <span className="text-[10px] opacity-60">{city.state}</span>
                  </button>
                ))}
              </div>
            )}
        ...
        <div className="sm:hidden border-t border-slate-200 bg-white p-4 space-y-3 font-mono">
          <div className="p-3 rounded-xl bg-slate-50 border border-slate-200 space-y-2">
            <span className="text-[10px] font-bold uppercase text-slate-400 block">
              Priority Location Selector
            </span>
            <div className="flex flex-wrap gap-1.5">
              {SUPPORTED_CITIES.slice(0, 4).map((c) => (
                <button
                  key={c.id}
